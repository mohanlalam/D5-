 ...
 * Live Scores powered by ESPNCricinfo Public Live Feed (Unlimited & Zero-Key)
 */
...
function App() {
  ...
  // ESPNCricinfo Live Feed State
  const [espnMatches, setEspnMatches] = useState([]);
  const [espnLoading, setEspnLoading] = useState(false);
  const [lastEspnUpdated, setLastEspnUpdated] = useState(null);
  ...
  // ── ESPNCricinfo Public Feed Fetcher ───────────────────────────────────────
  const fetchEspnLiveScores = async () => {
    setEspnLoading(true);
    try {
      // 1. Try local server API first, fallback to public proxy
      let xmlText = "";
      try {
        const localRes = await fetch("/api/v1/live/scores");
        const localData = await localRes.json();
        if (localData.data && Array.isArray(localData.data)) {
          setEspnMatches(localData.data);
          setLastEspnUpdated(new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }));
          setEspnLoading(false);
          return;
        }
      } catch(e) {}

      // 2. Direct browser fallback via public CORS bridge
      const rssUrl = encodeURIComponent("https://static.cricinfo.com/rss/livescores.xml");
      ...
      setEspnMatches(parsed);
      setLastEspnUpdated(new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }));
    } catch(err) {
      console.warn("ESPNCricinfo live feed:", err.message);
    } finally {
      setEspnLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchEspnLiveScores();
    const interval = setInterval(fetchEspnLiveScores, 30000); // 30-sec auto-refresh
  ...
  return h("div", { style: { minHeight: "100vh", display: "flex", flexDirection: "column" } },
      ...
      h("div", {
        ...
        h("div", {
          ...
          espnMatches.length > 0
            ? h("span", { style: { color: "var(--accent-primary)", fontWeight: "600" } }, `ESPN: ${espnMatches[0].title.slice(0, 32)}...`)
            : simActive
            ? h("span", { style: { color: "var(--accent-primary)", fontWeight: "600" } }, `${simScore.team1} ${simScore.r}/${simScore.w} (${simScore.o} ov)`)
            : h("span", { style: { color: "var(--text-secondary)" } }, "ESPNCricinfo Live Feed Connected")
        ),
    ...
    h("main", { style: { flex: 1, maxWidth: "1240px", width: "100%", margin: "0 auto", padding: "24px 20px" } },
      ...
      // 📡 View 3: Live Feed (ESPNCricinfo Public Feed + Simulator)
      tab === "live" && h("div", null,
        h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "18px", flexWrap: "wrap", gap: "10px" } },
          h("div", null,
            h("h1", { style: { fontSize: "24px", fontWeight: "700", letterSpacing: "-0.5px" } }, "ESPNCricinfo Live Feed & Schedule"),
            lastEspnUpdated && h("p", { style: { fontSize: "13px", color: "var(--text-secondary)" } }, `Live feed updated at ${lastEspnUpdated} (Unlimited Zero-Key RSS)`)
          ),
          h("div", { style: { display: "flex", gap: "8px" } },
            h(AppButton, {
              ...
              small: true,
              loading: espnLoading,
              onClick: fetchEspnLiveScores,
            }, "🔄 Refresh ESPN Feed"),
            h(AppButton, {
              variant: simActive ? "danger" : "primary",
              small: true,
              onClick: () => setSimActive(!simActive)
            }, simActive ? "⏹ Stop Sim" : "▶ Start Live Sim")
          )
        ),

        // Live ESPNCricinfo Cards Grid
        espnMatches.length > 0 && h("div", { style: { marginBottom: "24px" } },
          h("h2", { style: { fontSize: "16px", fontWeight: "700", color: "var(--accent-primary)", marginBottom: "12px" } },
            `📡 Live Matches from ESPNCricinfo (${espnMatches.length})`
          ),
          h("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "12px" } },
            espnMatches.map((m, idx) => h(AppCard, { key: idx },
              h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "8px" } },
                h("div", { style: { fontWeight: "700", fontSize: "15px", color: "#fff" } }, m.title),
                h("span", { className: "badge-pill badge-live" }, "ESPN LIVE")
              ),
              m.description && h("div", { style: { fontSize: "12.5px", color: "var(--text-secondary)", lineHeight: "1.5", marginTop: "4px" } }, m.description),
              m.link && h("a", {
                href: m.link,
                target: "_blank",
                rel: "noopener noreferrer",
                style: { display: "inline-block", fontSize: "12px", color: "var(--accent-secondary)", marginTop: "8px", textDecoration: "none", fontWeight: "600" }
